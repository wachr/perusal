...
export function editByPath(topic, path, calculateNewValue) {
  ...
  return path.reduce((currentTopic, pathElement) => {
    if (!currentTopic) return currentTopic;
    if (!pathElement) return calculateNewValue(currentTopic);
    if (pathElement === currentTopic || pathElement === currentTopic.topicTitle)
    ...
    return (subtopic && calculateNewValue(subtopic)) || null;
  }, topic);
}
